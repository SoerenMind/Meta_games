...
def run_linear():
    independent_vars = ['seed', 'lr_out', 'n_outer_opt']    # For graph titles
    ...
    lr_in = '10'
    lr_out = '1'
    net_type = 'OppAwareLinearNetSubspace'
    layer_sizes = ('10')

    # File handling
    exp_group_name = '2018-10-09 16h: LINEAR'
    foldername = exp_group_name
    start_logger(exp_group_name)

    num_runs = np.product([len(opt) for opt in [seed_opt]])
    run_counter = 0

    for seed in seed_opt:
        run_counter += 1
        print('Run %i / %i' %(run_counter, num_runs))
        run(# Independent vars:
